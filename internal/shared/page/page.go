...
// ToSpecPagination converts to shared.Pagination
func (p PaginationRequest) ToPagination() Pagination {
	return Pagination{
		Number:  int(p.Page),
		Size:    int(p.PageSize),
...
// FromSpecPagination creates a PaginationRequest from Pagination
func FromSpecPagination(pagi Pagination) PaginationRequest {
	sortDir := ASC
	if pagi.SortDir != "" {
	...
	return PaginationRequest{
		Page:          int32(pagi.Number),
		PageSize:      int32(pagi.Size),
