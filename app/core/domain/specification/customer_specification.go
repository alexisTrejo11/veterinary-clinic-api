...
func (s *CustomerSpecification) IsSatisfiedBy(entity any) bool {
	...
	if s.Name != nil && !strings.Contains(strings.ToLower(customer.FullName().FirstName), strings.ToLower(*s.Name)) {
		return false
	}

	if s.LastName != nil && !strings.Contains(strings.ToLower(customer.FullName().LastName), strings.ToLower(*s.LastName)) {
		return false
	}
