...
func (h *userEventHandler) processUserRegistration(event UserRegisteredEvent) {
	...
	profileID, err := h.service.CreateProfile(ctx, event.UserID)
	if err != nil {
		h.eventLog.LogOperationError(ctx, err, "Create profile failed",
			zap.String("sub_operation", "create_profile"))
		return

	}
	profileIDStr := strconv.FormatUint(uint64(profileID), 10)

	h.eventLog.LogOperationSuccess(ctx, "Profile created",
		zap.String("profile_id", profileIDStr),
		zap.String("sub_operation", "create_profile"))

	if err := h.service.AttachProfileToUser(ctx, event.UserID, profileID); err != nil {
		h.eventLog.LogOperationError(ctx, err, "Attach profile failed",
			zap.String("profile_id", profileIDStr),
			zap.String("sub_operation", "attach_profile"))
		return
	}
	h.eventLog.LogOperationSuccess(ctx, "Profile attached",
		zap.String("profile_id", profileIDStr),
		zap.String("sub_operation", "attach_profile"))
...
func (h *userEventHandler) handleCustomerRegistration(ctx context.Context, event UserRegisteredEvent, profileID string) {
...
func (h *userEventHandler) handleEmployeeRegistration(ctx context.Context, event UserRegisteredEvent, profileID string) {
