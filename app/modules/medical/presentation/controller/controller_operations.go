...
func NewMedSessionControllerOperations(bus *bus.MedicalSessionBus, validator *validator.Validate) *MedSessionControllerOperations {
...
func (co *MedSessionControllerOperations) GetMedSessionsByID(c *gin.Context, getByIDExtraArgs *GetByIDExtraArgs) {
	...
	result, err := co.bus.QueryBus.FindMedSessionByID(c.Request.Context(), *getByIDQuery)
	if err != nil {
...
func (co *MedSessionControllerOperations) GetMedSessionsByPetID(c *gin.Context, petID uint, customerID *uint) {
	...
	resultPage, err := co.bus.QueryBus.FindMedSessionByPetID(c.Request.Context(), *query)
	if err != nil {
		response.ApplicationError(c, err)
		return
	}
...
func (co *MedSessionControllerOperations) SoftDeleteMedicalSession(c *gin.Context) {
	idUint, err := ginUtils.ParseParamToUInt(c, "id")
	if err != nil {
		response.BadRequest(c, httpError.RequestURLParamError(err, "medical-history", c.Param("id")))
		return
