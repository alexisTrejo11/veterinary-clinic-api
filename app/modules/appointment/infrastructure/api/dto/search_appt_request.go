...
type AppointmentSearchRequest struct {
	OwnerID   uint   `form:"owner_id" validate:"omitempty,min=1"`
	VetID     uint   `form:"vet_id" validate:"omitempty,min=1"`
...
func (r *AppointmentSearchRequest) ToSpecification() (*specification.ApptSearchSpecification, error) {
	...
	if r.OwnerID > 0 {
		ownerID := valueobject.NewOwnerID(r.OwnerID)
		spec = spec.WithOwnerID(ownerID)
	}

	// Vet ID
	if r.VetID > 0 {
		vetID := valueobject.NewVetID(r.VetID)
		spec = spec.WithVetID(vetID)
	}
