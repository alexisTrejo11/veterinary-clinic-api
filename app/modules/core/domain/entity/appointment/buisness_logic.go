...
func (a *Appointment) Update(ctx context.Context, notes *string, service *enum.ClinicService) error {
...
func (a *Appointment) Confirm(ctx context.Context, employeeID vo.EmployeeID) error {
...
func (a *Appointment) ValidatePersistence(ctx context.Context) error {
...
func (a *Appointment) UpdateStatus(ctx context.Context, newStatus enum.AppointmentStatus) error {
	if !newStatus.IsValid() {
		return domainerr.ValidationError(ctx, "APPOINTMENT_VALIDATION", "appointment", "status", "invalid appointment status", "UpdateStatus")
	}
...
func (a *Appointment) UpdateNotes(ctx context.Context, notes *string) error {
	if notes != nil && len(*notes) > 1000 {
		return domainerr.ValidationError(ctx, "APPOINTMENT_VALIDATION", "appointment", "notes", "notes too long (max 1000 characters)", "UpdateNotes")
	}
