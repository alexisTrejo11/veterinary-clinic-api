...
func (f *TokenFactory) CreateToken(tokenType vo.TokenType, config TokenConfig) (vo.Token, error) {
	if config.Secret == nil {
		config.Secret = f.jwtSecret
	}

	switch tokenType {
	case vo.JWTAccessToken:
		if config.ExpiresIn == 0 {
			config.ExpiresIn = 15 * time.Minute // Default: 15 min
		}
		return &JWTAccessTokenImpl{config: config}, nil

	case vo.JWTRefreshToken:
		if config.ExpiresIn == 0 {
