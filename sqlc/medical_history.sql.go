...
const listMedicalHistories = `-- name: ListMedicalHistories :many
SELECT id, pet_id, date, description, vet_id, created_at, updated_at
FROM medical_histories
ORDER BY id
`

func (q *Queries) ListMedicalHistories(ctx context.Context) ([]MedicalHistory, error) {
	rows, err := q.db.Query(ctx, listMedicalHistories)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
