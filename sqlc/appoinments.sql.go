...
const hardDeleteAppointment = `-- name: HardDeleteAppointment :exec
DELETE FROM appointments WHERE id = $1
`

func (q *Queries) HardDeleteAppointment(ctx context.Context, id int32) error {
	_, err := q.db.Exec(ctx, hardDeleteAppointment, id)
	return err
}

const restoreAppointment = `-- name: RestoreAppointment :exec
UPDATE appointments SET
    deleted_at = NULL
WHERE id = $1
`

func (q *Queries) RestoreAppointment(ctx context.Context, id int32) error {
	_, err := q.db.Exec(ctx, restoreAppointment, id)
	return err
}

const softDeleteAppointment = `-- name: SoftDeleteAppointment :exec
UPDATE appointments SET
    deleted_at = CURRENT_TIMESTAMP
WHERE id = $1
`

func (q *Queries) SoftDeleteAppointment(ctx context.Context, id int32) error {
	_, err := q.db.Exec(ctx, softDeleteAppointment, id)
	return err
}
